...
class DilatedConv(nn.Module):
    def __init__(self, train_shape, category, kernel_size=9, dilations=[1, 2, 3, 5]):
        ...
        self.layer = nn.Sequential(
            nn.Conv2d(1, 64, (kernel_size, 1), (2, 1), ((dilations[0] * (kernel_size - 1) + 1) // 2, 0), dilation=dilations[0]),
            nn.BatchNorm2d(64),
            nn.ReLU(),

            nn.Conv2d(64, 128, (kernel_size, 1), (2, 1), ((dilations[1] * (kernel_size - 1) + 1) // 2, 0), dilation=dilations[1]),
            nn.BatchNorm2d(128),
            nn.ReLU(),

            nn.Conv2d(128, 256, (kernel_size, 1), (2, 1), ((dilations[2] * (kernel_size - 1) + 1) // 2, 0), dilation=dilations[2]),
            nn.BatchNorm2d(256),
            nn.ReLU(),
